...
function BookkeepingSkills() {
  ...
  const fetchSkills = async () => {
    getSkills(skillsCategories.bookkeeping)
      .then((response) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (skillsToCreate.length || skillsToDelete.length) {
      ...
      updateSkills({ skills_to_update: { skills_to_create: skillsToCreate, skills_to_delete: skillsToDelete } })
        .then(() => {
          setSubmitting(false);
          history.push('/freelancer-application/accounting-skills');
        })
        .catch(() => {
          showSnackbar('Something went wrong', 'error');
          setSubmitting(false);
  ...
  return (
    <div className={classes.container}>
      <StepHeader title="Bookkeeping Skills" step={3} />
...
export default BookkeepingSkills;
