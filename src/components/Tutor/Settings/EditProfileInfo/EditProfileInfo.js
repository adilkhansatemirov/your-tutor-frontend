...
import { getFreelancer, uploadResume, updateFreelancerDetails } from 'services/freelancer/freelancers';
import { getFreelancerSkills, updateSkills } from 'services/freelancer/skills';
...
function EditProfileInfo() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const freelancerResponse = await getFreelancer();
        ...
        const skillsResponse = await getFreelancerSkills();
  ...
  const handleSubmit = async () => {
    ...
    try {
    await updateFreelancerDetails({ freelancer_detail: freelancerDetails });
      if (acceptedFiles[0]) {
